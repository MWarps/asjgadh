...
function knoppenFunctie(){
    ...
    if(isset($_SESSION['gebruikersnaam'])){
        try {
            require('core/dbconnection.php');
            $sqlSelect = $dbh->prepare("SELECT verkoper FROM Gebruiker WHERE gebruikersnaam = :gebruikersnaam");
            $sqlSelect->execute(
                array (
                    ':gebruikersnaam' => $_SESSION['gebruikersnaam'],
                ));

            $verkoper = $sqlSelect->fetchAll(PDO::FETCH_ASSOC);
    ...
    if ($_SESSION['ingelogd']){
        ...
        if ($verkoper['verkoper'] == 1 ){
            echo '<a class="dropdown-item" href="verkoper.php">Wordt verkoper</a>';
        }  else {
            echo '<a class="dropdown-item" href="#">Veiling starten</a>';
        }
        '<a class="dropdown-item" href="#">Verkoper</a>
